...
function validations(c::Collar, s::Survey, intervals::Intervals)
    ...
    # read files
    dfc =
        c.file isa String ? CSV.read(c.file, DataFrame, select = cfields) :
        select(c.file, cfields)
    dfs =
        s.file isa String ? CSV.read(s.file, DataFrame, select = sfields) :
        select(s.file, sfields)
    dfi = []
    for f in it
        ffields = [f.holeid, f.from, f.to]
        d =
            f.file isa String ? CSV.read(f.file, DataFrame, select = ffields) :
            select(f.file, ffields)
        push!(dfi, d)
    end
    ...
    # check for overlaps
    ovlp = [overlaps(dfi[x], it[x]) for x = 1:length(it)]
    ...
    for x = 1:length(it)
        sum(nani[x]) > 0 &&
            push!(out, ("Error", fname(it[x].file), outnan(dfi[x], nani[x])))
        sum(typi[x]) > 0 &&
            push!(out, ("Error", fname(it[x].file), outtyp(dfi[x], typi[x])))
        sum(typi[x]) == 0 &&
            length(ovlp[x]) > 0 &&
            push!(out, ("Error", fname(it[x].file), outovl(ovlp[x])))
    end
