...
function fillxyz!(tab, trace, pars; output = ["mid"])
    ...
    for i = 1:size(tab, 1)
        ...
        for out in output
            ...
            ismissing(atx) && error(
                "Missing info; and output kw must be a list containing \"mid\", \"from\" or \"to\"",
            )
            ...
            if d1x == 0
                ...
                azx, dpx =
                    b[1] == b[2] ? (az2, dp2) :
                    weightedangs([az1, dp1], [az2, dp2], d12, d1x)
                dx, dy, dz =
                    tang ? tangential(az1, dp1, d1x) : mincurv(az1, dp1, azx, dpx, d1x)
