...
oweight(object, method::BlockWeighting, offsets::Int; kwargs...) =
    oweight(domain(object), method, offsets; kwargs...)
...
function cell_declus_tests(
    comps::AbstractGeoTable,
    evar,
    iterator;
    offsets = 3,
    mode = "simple",
    ...
    fun =
        Transducers.Map(spacing -> cell_declus_tests_i(comps, evar, spacing, offsets, mode))
...
function blocks_iterator(; x = nothing, y = nothing, xy = nothing, z = nothing)
    ...
    mapreduce(vcat, iterators) do i
        #println(i)
        is2d && issquare ? (i[1], i[1]) :
        is2d ? (i[1], i[2]) : issquare ? (i[1], i[1], i[2]) : (i[1], i[2], i[3])
    end
end
...
function get_spacing(obj; coords = [:x, :y, :z])
    ...
    elseif obj isa AbstractDataFrame
        dx = mapreduce(vcat, coords) do c
            ...
            vals = [vals[i] - vals[i-1] for i = 2:length(vals)]
            minimum(vals)
        end
        tuple(dx...)
    else
...
function downscale(geotable::AbstractGeoTable, factors; blks = true, ds = nothing)
    ...
    new_geoms = foldxt(
        vcat,
        Transducers.Map(x -> outfun(x, hs, isub, subd)),
...
function downscale(table::AbstractDataFrame, factors; blks = true, coords = [:x, :y, :z])
    ...
    cpairs = mapreduce(vcat, enumerate(coords)) do (ic, c)
        cvals = [ustrip(to(centroid(dom, i))[ic]) for i = 1:nelements(dom)]
        c => cvals
    end
...
function new_subblocks(geotable, hs, isub, subd)
    new_geoms = mapreduce(vcat, zip(geotable.geometry, geotable.IJK)) do (g, ijk)
        ...
        subblks = mapreduce(vcat, origins) do o
            minmax = [(o[x], o[x] + subd[x]) for x = 1:length(subd)]
...
function new_subpts(geotable, hs, isub, subd)
    new_geoms = mapreduce(vcat, zip(geotable.geometry, geotable.IJK)) do (g, ijk)
        ...
        subpts = mapreduce(vcat, origins) do o
            coords = [mean([o[x], o[x] + subd[x]]) for x = 1:length(subd)]
            Point(coords...)
        end
...
function regblks_to_subblks(ptable, model, doms_filter, output)
    ...
    suffixes =
        ...
        output == "mean" ? ["_mean"] :
        error("output must be \"full\", \"mean\" or a vector of quantiles ranging [0,1]")
    ...
    for group in est_group
        ...
        if output isa AbstractVector
            ...
            for i = 1:length(output)
                emodel[!, colnames[i]] =
                    [quantile(row, output[i]) for row in eachrow(emodel[!, vars])]
            end
