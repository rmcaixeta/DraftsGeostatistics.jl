...
function AdvBallSearch(
  ...
  AdvBallSearch{D,B,typeof(tree)}(domain, k, ball, maxpercategory, usesectors, rank_metric, tree)
end

function AdvBallSearch(geotable::GeoTable, ball; k=1000, maxpercategory=nothing, usesectors=nothing, rank_metric=:same)
  if !isnothing(maxpercategory)
    if !hasproperty(maxpercategory, :geotable)
      maxpercategory = (maxpercategory..., geotable=geotable[:, keys(maxpercategory)])
    end
  end
...
function GeoStats.searchdists!(neighbors, distances, pₒ::Point, method::AdvBallSearch; mask=nothing)
  ...
  dists = [evaluate(rank_metric, ustrip.(to(pₒ)), ustrip.(to(centroid(domain, ind)))) for ind in inds]
...
function initsectors(meth)
  ...
  Dict(:use => true, :count => zeros(Int, ns), :rotmat => rotmat, :max => maxpersector, :n => (pars.n, pars.split))
end
