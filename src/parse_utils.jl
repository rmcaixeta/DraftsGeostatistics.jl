...
function rounded_str(x)
    ...
    10 <= x < 100 ? @sprintf("%.1f", x) :
    100 <= x < 1000 ? @sprintf("%.0f", x) : @sprintf("%.2e", x)
end

evalstr(x) = eval(Meta.parse(x))

get_filters(tab::AbstractDataFrame, col::Symbol) =
    [evalstr("row -> " * ln[col]) for ln in eachrow(tab)]
get_filters(rules::AbstractVector{String}) =
    Dict(d => evalstr("row -> " * d) for d in rules)
